...
public class Emoji {
	...
	/**
	 * A negative look-behind ensuring that the match is not preceded by one of the
	 * characters above
	 */
	private static final String REGEX_NEGATIVE_LOOKBEHIND = "(?<!" + REGEX_SURROUNDING_CHARS_DISALLOWED + ")";
	/**
	 * A negative look-ahead ensuring that the match is not followed by one of the
	 * characters above
	 */
	...
	/**
	 * Constructs a regular expression which ensures that the emoticon is not part
	 * of a longer string of special chars (e.g. <:)))> or <http://> which both
	 * include basic emoticons)
	 */
	...
	/**
	 * Converts between emoticons and their corresponding Unicode Emoji in the given
	 * text
	 *
	 * @param text
	 *            the String to search and replace in
	 * @param reverse
	 *            whether to replace emoticons with emoji as usual (false) or revert
	 *            emoji to emoticons (true)
